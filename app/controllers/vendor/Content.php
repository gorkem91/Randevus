...
class Content extends CI_Controller {
    ...
    public function forgot_password_action() {
        ...
        if ($vendor_data['errorCode'] == 1) {
            ...
            $define_param['to_name'] = ucfirst($vendor_data['first_name']) . " " . ucfirst($vendor_data['last_name']);
            $define_param['to_email'] = $vendor_data['email'];
            $userid = $vendor_data['id'];
            $hidenuseremail = $vendor_data['email'];
            $hidenusername = ucfirst($vendor_data['first_name']) . " " . ucfirst($vendor_data['last_name']);
            //Encryprt data
            $encid = $this->general->encryptData($userid);
            $encemail = $this->general->encryptData($hidenuseremail);
            $url = $this->config->item("site_url") . "vendor/reset-password/" . $encid . "/" . $encemail;
                                    ...
                                    <tbody>
                                        <tr>
                                            <td style = "text-align:center; margin:0 auto;">
                                                <br>
                                                    <div>
                                                    <a href = "' . $url . '" style = "background-color:#f5774e;color:#ffffff;display:inline-block;font-family:Helvetica, Arial, sans-serif;font-size:18px;font-weight:400;line-height:45px;text-align:center;text-decoration:none;width:220px;-webkit-text-size-adjust:none;">' . translate('reset_password') . '</a>
                                                    </div>
                                                <br>
                                            </td>
                                        </tr>
    ...
    public function profile_save() {
        $this->authenticate->check_vendor();
        $vendor_id = (int) $this->session->userdata('Vendor_ID');
        $this->form_validation->set_rules('first_name', '', 'required|max_length[50]');
        $this->form_validation->set_rules('last_name', '', 'required|max_length[50]');
        $this->form_validation->set_rules('email', '', 'required');
        $this->form_validation->set_rules('profile_text', '', 'required');
        $this->form_validation->set_rules('phone', '', 'required|min_length[10]|is_unique[app_admin.phone.id.' . $vendor_id . '] ');
        $this->form_validation->set_message('required', translate('required_message'));
        $this->form_validation->set_error_delimiters('<div class = "error"> ', '</div>');
        if ($this->form_validation->run() == false) {
            $this->profile();
