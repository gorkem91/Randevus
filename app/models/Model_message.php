...
class Model_message extends CI_Model {
    ...
    function message_customer_list($login_id) {
        $this->db->select('app_chat_master.*,app_chat.msg_read,app_chat.to_id,app_chat.chat_id,app_customer.profile_image,app_customer.first_name,app_customer.last_name,app_customer.id as vendor_id');
    ...
    function msg_group_list($vendor_id, $customer_id) {
        $this->db->select('DATE(app_chat.created_on) as date,chat_id');
        ...
        $this->db->group_by("DATE(app_chat.created_on)");
